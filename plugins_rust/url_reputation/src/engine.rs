use crate::filters::{heuristic, patterns};
use anyhow::Result;
use pyo3::prelude::*;
use std::{
...
#[pyfunction]
pub fn validate_url(url: &str, config: &URLReputationConfig) -> Result<URLPluginResult> {
    ...
    if config.block_non_secure_http && scheme != "https" {
        return Ok(URLPluginResult {
            continue_processing: false,
            violation: Some(PluginViolation {
    ...
    if patterns::in_blocked_list(domain, &config.blocked_domains) {
        return Ok(URLPluginResult {
            continue_processing: false,
            violation: Some(PluginViolation {
    ...
    if patterns::in_blocked_patterns(url, &config.blocked_patterns) {
        return Ok(URLPluginResult {
            continue_processing: false,
            violation: Some(PluginViolation {
    ...
    if !ip_domain && config.use_heuristic_check {
        if !heuristic::passed_entropy(domain, config.entropy_threshold) {
            return Ok(URLPluginResult {
                continue_processing: false,
                violation: Some(PluginViolation {
...
mod tests {
    ...
    fn test_whitelisted_domain() {
        ...
        let result = validate_url(url, &config).unwrap();
    ...
    fn test_non_secure_http() {
        let config = URLReputationConfig {
            whitelist_domains: HashSet::from([]),
            blocked_domains: HashSet::from([]),
            blocked_patterns: HashSet::from([]),
        ...
        assert!(result.violation.unwrap().reason == "Blocked non secure http url".to_string())
    }
    ...
    fn test_blocked_domain() {
        let config = URLReputationConfig {
            whitelist_domains: HashSet::from([]),
            blocked_domains: HashSet::from(["example.com".to_string()]),
            blocked_patterns: HashSet::from([]),
            use_heuristic_check: false,
            entropy_threshold: 0.0,
            block_non_secure_http: true,
        ...
        let result = validate_url(url, &config).unwrap();
        assert!(false == result.continue_processing);
        assert!(result.violation.unwrap().reason == "Blocked domain".to_string());
    }
    ...
    fn test_blocked_pattern() {
        let config = URLReputationConfig {
            whitelist_domains: HashSet::from([]),
            blocked_domains: HashSet::from([]),
            blocked_patterns: HashSet::from(["crypto".to_string()]),
            use_heuristic_check: false,
            entropy_threshold: 0.0,
            block_non_secure_http: true,
        };
        ...
        let result = validate_url(url, &config).unwrap();
        assert!(false == result.continue_processing);
        assert!(result.violation.unwrap().reason == "Blocked pattern".to_string());
    }
    ...
    fn test_heuristic_domain_too_long() {
        ...
        let result = validate_url(&url, &config).unwrap();
        assert!(false == result.continue_processing);
        assert!(result.violation.unwrap().reason == "Domain unicode is not secure".to_string());
    }
    ...
    fn test_is_domain_unicode_secure_mixed_scripts() {
        ...
        let url = "https://pаypal.com/test"; // cirilic a
        let result = validate_url(url, &config).unwrap();
        assert!(false == result.continue_processing);
        assert!(result.violation.unwrap().reason == "Domain unicode is not secure".to_string());
    }
    ...
    fn test_url_valid_ipv4() {
        ...
        let result = validate_url(url, &config).unwrap();
        // println!("REASON {}", result.violation.unwrap().reason);
        assert!(true == result.continue_processing);
    }
